"""
Template for rhoSimpleFoam solver.
Steady-state compressible turbulent flow using the SIMPLE algorithm.

Key differences from simpleFoam:
  - Pressure (p) in Pa, dimensions [1 -1 -2 0 0 0 0]
  - Temperature field (T) with energy equation
  - Turbulent thermal diffusivity (alphat)
  - thermophysicalProperties replaces transportProperties
  - Compressible divergence schemes: div(phi,e), div(phi,K), div(phi,Ekp)
  - Energy residual control in fvSolution
...
SOLVER_DESCRIPTION = "Steady-state solver for compressible, turbulent flow (SIMPLE algorithm)"

# Base fields that are always active (not dependent on turbulence model)
BASE_FIELDS = ["p", "U", "T"]
...
# Field metadata for per-patch BC generation (dimensions, class, default internal)
FIELD_INFO = {
...
CONTROL_DICT = {
    ...
    "groups": {
        "Time Control": [
            ("Application", "application", "rhoSimpleFoam", "str", None),
...
# ------------------------------------------------------------------ #
#  fvSolution — with energy equation
# ------------------------------------------------------------------ #
...
FV_SOLUTION = {
    ...
    "groups": {
        ...
        "Relaxation Factors": [
            ...
            ("e Relaxation", "relaxE", 0.5, "float", (0.0, 1.0)),
            ("rho Relaxation", "relaxRho", 0.5, "float", (0.0, 1.0)),
    ...
    "groups": {
        "Thermo Type": [
            ...
            ("Equation of State", "equationOfState", "perfectGas", "combo",
             ["perfectGas", "rhoConst", "incompressiblePerfectGas", "PengRobinsonGas"]),
...
BC_P = {
    ...
    "groups": {
        "Pressure Field [Pa]": [
            ("Internal Field [Pa]", "pInternal", 101325, "float", (0, 1e9)),
            ("Outlet Value [Pa]", "pOutlet", 101325, "float", (0, 1e9)),
        ],
    },
}
...
BC_T = {
    ...
    "groups": {
        "Temperature Field": [
            ("Internal / Inlet Value [K]", "TInternal", "300", "str", None),
        ],
    },
}
...
BC_ALPHAT = {
    ...
    "groups": {
        "alphat Field": [
            ("Internal Value [kg/(m·s)]", "alphatInternal", "0", "str", None),
        ],
    },
}

# ---- blockMeshDict (always present) ---- #
...
# ---- Mesh dictionaries (conditionally included with STL) ---- #
...
def get_base_dicts():
    """Return the always-present dictionaries for rhoSimpleFoam."""
    return [
        CONTROL_DICT,
        FV_SCHEMES,
        FV_SOLUTION,
