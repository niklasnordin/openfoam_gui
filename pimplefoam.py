"""
Template for pimpleFoam solver.
Transient solver for incompressible, turbulent flow using the PIMPLE algorithm
(merged PISO-SIMPLE).

Key differences from simpleFoam:
  - Transient: ddt scheme is Euler/CrankNicolson, not steadyState
  - PIMPLE algorithm: nOuterCorrectors, nCorrectors, pFinal/UFinal solvers
  - Adjustable time stepping with maxCo control
  - Relaxation factors typically 1.0 for outer correctors > 1
...
SOLVER_DESCRIPTION = "Transient solver for incompressible, turbulent flow (PIMPLE algorithm)"
...
# ------------------------------------------------------------------ #
# controlDict — transient with adjustable time step
# ------------------------------------------------------------------ #
...
CONTROL_DICT = {
    ...
    "groups": {
        "Time Control": [
            ("Application", "application", "pimpleFoam", "str", None),
        ...
        "Write Control": [
            ...
            ("Write Interval", "writeInterval", 0.1, "float", (1e-12, 1e9)),
...
FV_SOLUTION = {
    ...
    "groups": {
        "Pressure Solver": [
            ...
            ("pFinal Tolerance", "pFinalTol", "1e-7", "str", None),
        ],
        "Velocity Solver": [
    ...
    "groups": {
        "Fluid Properties": [
            ("Transport Model", "transportModel", "Newtonian", "combo",
             ["Newtonian", "CrossPowerLaw", "BirdCarreau", "HerschelBulkley"]),
...
TURBULENCE_PROPERTIES = {
    ...
    "groups": {
        ...
        "RAS Settings|simulationType=RAS": [
            ("RAS Model", "RASModel", "kOmegaSST", "combo",
...
def get_base_dicts():
    """Return the always-present dictionaries for pimpleFoam."""
    return [
        CONTROL_DICT,
        FV_SCHEMES,
        FV_SOLUTION,
...
def get_turbulence_fields(model_name: str) -> list[str]:
    ...
    info = TURBULENCE_MODELS.get(model_name, TURBULENCE_MODELS["kOmegaSST"])
    return info["fields"]
